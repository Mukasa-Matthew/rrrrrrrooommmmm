...
router.get('/', async (req, res) => {
  try {
    ...
    const r = await pool.query(
      `SELECT * FROM expenses WHERE hostel_id = $1 ORDER BY spent_at DESC
...
router.post('/', async (req: Request, res) => {
  try {
    ...
    const r = await pool.query(
      `INSERT INTO expenses (hostel_id, user_id, amount, currency, category, description, spent_at)
       VALUES ($1, $2, $3, $4, $5, $6, COALESCE($7, NOW())) RETURNING *`,
      [hostelId, currentUser.id, parseFloat(amount), currency || 'UGX', category || null, description || null, spent_at || null]
    );
